...
class Client extends EventEmitter {
            ...
            async () => {
                ...
                try {
                    ...
                    if (!injected) {
                        ...
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: Store or WWebJs is undefined, so initialising"
                        );

                        if (isCometOrAbove) {
                            // make sure the pushname module is ready before injection
                            await this.pupPage.evaluate(async () => {
                                const WWebSetPushnameConnAction =
                                ...
                                if (!WWebSetPushnameConnAction) {
                                    console.warn(
                                        "onAppStateHasSyncedEvent: WAWebSetPushnameConnAction not found. WAWebProfileDrawerLoadableRequireBundle will be called to try loading it."
                                    );
                        ...
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: Store Exposed started, waiting for completion"
                        );
                        ...
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: Store Exposed completed, window.Store defined"
                        );
                        ...
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: Got user info"
                        );
                        ...
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: InterfaceController constructed"
                        );
                        ...
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: Utils loaded"
                        );
                        ...
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: event listeners attached, next is ready state"
                        );
                    }
                } catch (err) {
                    if (err.message) {
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: Error whilst processing: " +
                                err.message
                        );
                    } else {
                        this.emit(
                            Events.SPHERE_LOG,
                            "onAppStateHasSyncedEvent: Error whilst processing. See logs for how far the code got"
                        );
                    }
                    throw err;
                }
    ...
    async sendSphereMessage(chatId, content, options = {}) {
        ...
        const sentMsgId = await this.pupPage.evaluate(
            async (chatId, content, options) => {
                ...
                const msgId = await window.WWebJS.sendSphereMessage(
                    chat,
                    content,
                    options
                );
    ...
    async getPollVotes(messageId) {
        ...
        const pollVotes = await this.pupPage.evaluate(async (msg) => {
            ...
            let pollVotes =
                await window.Store.PollsVotesSchema.getTable().equals(
                    ["parentMsgKey"],
                    msgKey.toString()
                );
